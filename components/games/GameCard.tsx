...
export default function GameCard({ game, priority = false }: GameCardProps) {
  return (
    <div className="group relative bg-white rounded-3xl border border-[#E4ECE7] overflow-hidden shadow-luxury hover:shadow-luxury-hover transition-all duration-300 transform hover:-translate-y-1.5 flex flex-col">
      {/* Visual Artwork Container */}
      <Link href={`/games/${game.slug}`} className="block relative w-full aspect-[4/3] overflow-hidden bg-[#F7FBF8]">
        <Image
          src={game.thumbnail}
          alt={game.name}
          fill
        ...
        <div className="absolute top-3 right-3 z-10">
          <div className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-white/95 backdrop-blur-sm text-xs font-bold text-[#172331] border border-[#E4ECE7] shadow-xs">
